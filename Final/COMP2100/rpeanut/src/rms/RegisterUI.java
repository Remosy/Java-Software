...
public class RegisterUI extends JPanel implements MouseListener, Register {
	...
	public static Register createReg(boolean term, String name) {
		if (term) {
			return new RegisterTerm();
		} else {
			return new RegisterUI(name);
		}
	}
	
	public RegisterUI(String name) {
		this.name = name;
		value = new Word();
	...
	public RegisterUI(String name, int index) {
		this.name = name;
		this.index = index;
		value = new Word();
		this.setLayout(new BoxLayout(this, BoxLayout.X_AXIS));
